...
def one_hot(indices, depth):
  ...
  index = indices.reshape(indices.size() + torch.Size([1]))  # (m, 1)
  encoded_indicies = encoded_indicies.scatter_(1, index, 1)
  return encoded_indicies


class CPN_LS(nn.Module):  # CPN + Label Smoothing
  def __init__(self, backbone, aug_num, alpha):
    super(CPN_LS, self).__init__()
  ...
  def forward(self, x, temp=5.):  # x:[batch, aug_num, C, H, W]
    ...
    targets_one_hot = one_hot(targets, batch)
    targets_one_hot = targets_one_hot*(1.-self.alpha)+(1.-targets_one_hot)*self.alpha/(batch-1)
    log_prb = F.log_softmax(logits/temp, dim=1)
    loss = -(targets_one_hot*log_prb).sum(dim=1).mean()
    return loss
