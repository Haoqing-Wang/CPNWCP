...
class CPN_CP(nn.Module):  # CPN + Confidence Penalty
  def __init__(self, backbone, aug_num):
    super(CPN_CP, self).__init__()
  ...
  def forward(self, x, temp=5.):  # x:[batch, aug_num, C, H, W]
    ...
    cp_loss = (probs*torch.log(probs+1e-12)).sum(1).mean()+np.log(batch)

    loss = ce_loss + cp_loss
    return loss
